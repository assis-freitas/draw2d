...
class CommandStack {
    ...
    private transactionCommand: any;
   ...
   * @returns {this}
   **/
  execute (command: { canExecute: () => boolean; execute: () => void; } | null) {
   ...
   * @since 4.0.0
   */
  startTransaction (commandLabel: any) {
    if (this.transactionCommand !== null) {
      debugger
   ...
   * @returns {this}
   */
  on (event: string, listener: { stackChanged: any; }) {
    if (event !== "change")
  ...
  removeEventListener (listener: any) {
     ...
     */
  off (listener: any) {
    this.eventListeners.grep((entry: { stackChanged: any; }) => (entry === listener || entry.stackChanged === listener))
    /*
   ...
   * @private
   **/
  notifyListeners (command: Command, state: any, action: string | undefined) {
...
//  Constant indicating notification after the stack has been (re)constructor(value is 64).
