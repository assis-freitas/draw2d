...
class FanConnectionAnchor extends  ConnectionAnchor {
   ...
   */
  constructor(owner: any, separation: string) {
    super(owner)
   ...
   */
  getLocation(reference: { x: number; y: number; distance: (arg0: any) => any; }, inquiringConnection: { getSource: () => any; getTarget: () => any; }) {
    ...
    switch (intersections.getSize()) {
      case 0:
        ...
        return v.asArray().reduce((previous: { distance: number; }, current: { distance: number; }) => {
...
export default FanConnectionAnchor;
