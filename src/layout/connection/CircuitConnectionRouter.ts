...
class CircuitConnectionRouter extends ManhattanConnectionRouter {
    private abortRoutingOnFirstVertexNode: boolean;
    private vertexRadius: number | undefined;
    private bridgeRadius: number | undefined;
    private bridge_LR: string | undefined;
    private bridge_RL: string | undefined;
  /**
   ...
   * @since 2.7.2
   */
  onUninstall(connection: { vertexNodes: { remove: () => void; } | null; }) {
   ...
   * @private
   * **/
  setJunctionRadius(radius: number | undefined) {
    this.vertexRadius = radius
  }
  /**
   *
   ...
   * @inheritdoc
   */
  route(conn: { getStartPoint: () => any; getSource: () => { (): any; new(): any; getConnectionDirection: { (arg0: any): any; new(): any; }; }; getTarget: () => { (): any; new(): any; getConnectionDirection: { (arg0: any): any; new(): any; }; }; getEndPoint: () => any; getCanvas: () => { (): any; new(): any; getIntersection: { (arg0: any): { (): any; new(): any; sort: { (arg0: string): any; new(): any; }; }; new(): any; }; }; vertexNodes: { remove: () => void; push: (arg0: any) => void; exclude: (arg0: any) => void; } | null; canvas: { paper: { set: () => any; ellipse: (arg0: any, arg1: any, arg2: number | undefined, arg3: number | undefined) => { (): any; new(): any; attr: { (arg0: { fill: any; }): any; new(): any; }; }; }; }; getVertices: () => any; sharingPorts: (arg0: any) => any; getZOrder: () => any; lineColor: { rgba: () => any; }; svgPathString: string; }, routingHints: any) {
    ...
    let oldP = p
    let bridgeWidth: number | null | undefined = null
    let bridgeCode: string | number | null | undefined = null

    let lastVertexNode: { remove: () => void; } | null = null
    ...
    for (let i = 1; i < ps.getSize(); i++) {
      ...
      //
      intersectionForCalc.each((ii: any, interP: { x: number; y: number; other: any; }) => {
        if (draw2d.shape.basic.Line.hit(1, oldP.x, oldP.y, p.x, p.y, interP.x, interP.y) === true) {
          ...
          if (conn.sharingPorts(interP.other)) {
            ...
            if (connZ < otherZ) {
              ...
              //
              if (this.abortRoutingOnFirstVertexNode) {
...
export default CircuitConnectionRouter;
