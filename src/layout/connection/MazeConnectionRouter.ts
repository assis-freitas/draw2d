...
class MazeConnectionRouter extends ConnectionRouter {
    protected useSpline: boolean;
    private useSimplify: boolean;
    protected useSimplifyValue: number;
    private useDebug: boolean;
    protected useShift: number;
    private portOutletOffset: number;
    protected finder: any;
  /**
...
//      this.finder = new PF.BreadthFirstFinder({ allowDiagonal: false});
   ...
   * @private
   */
  generateNoGoGrid(conn, fromPt, fromDir, toPt, toDir) {
    let shift = this.useShift
   ...
   */
  getAdjustedPoint(pt, direction, adjustment) {
  ...
  getSquareDistance(p1, p2) { // square distance between 2 points
  ...
  getSquareSegmentDistance(p, p1, p2) { // square distance from a point to a segment
  ...
  simplifyRadialDistance(points, sqTolerance) { // distance-based simplification
...
export default MazeConnectionRouter;
