 ...
 */
class SnapToGeometryEditPolicy extends SnapToEditPolicy {
  SNAP_THRESHOLD = 3;
  ...
  snapPoint (/*:int*/ snapOrientation, /*:draw2d.Point*/ inputPoint) {
  ...
  getCorrectionFor (/*:Array*/ entries, /*:double*/ value, /*:int*/ side) {
  ...
  showVerticalLine (causedFigure, edge, x) {
    if (this.vline != null) {
      this.vline.stop()
      this.vline.remove()
    }
  ...
  showHorizontalLine (causedFigure, edge, y) {
    if (this.hline != null) {
      this.hline.stop()
      this.hline.remove()
    }
...
export default SnapToGeometryEditPolicy;
