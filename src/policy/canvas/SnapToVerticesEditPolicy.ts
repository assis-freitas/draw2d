 ...
 */
class SnapToVerticesEditPolicy extends SnapToEditPolicy {
  SNAP_THRESHOLD = 3;
  ...
  getCorrectionFor (vertexResizeHandle, point) {
    var _this = this
  ...
  showVerticalLine (originalPos, snappedPos) {
    if (this.vline != null) {
      this.vline.stop()
      this.vline.remove()
    }
  ...
  showHorizontalLine (originalPos, snappedPos) {
    if (this.hline != null) {
      this.hline.stop()
      this.hline.remove()
    }
...
export default SnapToVerticesEditPolicy;
