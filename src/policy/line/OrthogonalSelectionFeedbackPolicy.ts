...
class OrthogonalSelectionFeedbackPolicy extends LineSelectionFeedbackPolicy {
  /**
  ...
  constructor(attr, setter, getter) {
    ...
    this.ResizeHandle = draw2d.ResizeHandle.extend(
       ...
       *        Sub classes can override this method to implement additional stuff. Don't forget to call the super implementation via <code>super();</code>
       *
   ...
   * @param segmentIndex
   */
  removeSegment (conn, segmentIndex) {
    let PADDING = 10
   ...
   * @param segmentIndex
   */
  splitSegment (conn, segmentIndex, x, y) {
    let segmentCount = conn.getVertices().getSize() - 1
    let p1 = conn.getVertex(segmentIndex)
    let p2 = conn.getVertex(segmentIndex + 1)
    let length = 40
   ...
   * @since 4.4.0
   */
  onRightMouseDown (conn, x, y, shiftKey, ctrlKey) {
...
export default OrthogonalSelectionFeedbackPolicy;
