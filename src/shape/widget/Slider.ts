...
class Slider extends Widget {
  ...
  onDragStart (x, y, shiftKey, ctrlKey) {
    ...
    if (this.slideBoundingBox.hitTest(x, y)) {
      ...
      tweenable.tween({
        from: {grow: this.thumbGrow}        to: {grow: 10}        duration: 500,
        easing: 'easeOutQuart',
        step (state) {
    ...
    return super(x, y, shiftKey, ctrlKey)
  }
  /**
   *
  ...
  onPanningEnd() {
    ...
    let tweenable = new Tweenable()
    tweenable.tween({
      from: {grow: this.thumbGrow}      to: {grow: 0}      duration: 300,
      easing: 'easeOutQuart',
      step (state) {
        this.thumbGrow = state.grow
        this.repaint()
