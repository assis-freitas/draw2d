...
class Figure {
    ...
    private id: any;
    private command: any;
    private isResizeHandle: boolean;
    private canvas: any;
    private shape: any;
    private children: ArrayList;
    ...
    private canSnapToHelper: boolean;
    private snapToGridAnchor: Point;
    ...
    private isInDragDrop: boolean;
    private repaintBlocked: boolean;
    private lastAppliedAttributes: any;
    private selectionHandles: ArrayList;
    private panningDelegate: any;
    private eventSubscriptions: any;
    private readonly relocateChildrenEventCallback: () => void;
    private readonly defaultSelectionAdapter: () => this;
    private selectionAdapter: () => this;
    ...
    constructor(attr: any) {
      ...
      this.snapToGridAnchor = new Point(0, 0)    // hot spot for snap to grid
     ...
     **/
    attr (name: string | object, value: object = {}) {
      return this
    }
    /**
     ...
     * @private
     */
    select (asPrimarySelection = true) {
      // apply all EditPolicy for select Operations
      //
      let _this = this
      this.editPolicy.each((e) => {
     ...
     */
    setSelectionAdapter (adapter?: () => this) {
      if (adapter == null) {
     ...
     */
    toggleCssClass (className: string) {
      className = className.trim()
      let newClass = ' ' + this.cssClass.replace(/[\t\r\n]/g, ' ') + ' '
      if (this.hasCssClass(className)) {
        while (newClass.indexOf(' ' + className + ' ') >= 0) {
    ...
    toFront (figure?: Figure) {
      ...
      if (!figure) {
        ...
        if (this.canvas) {
          ...
          if (figures.remove(this)) {
            figures.add(this)
          } else if (lines.remove(this)) {
            lines.add(this)
          }
        }
      } else {
      ...
      //
      this.selectionHandles.each( (handle) =>{
        handle.toFront()
      })
    ...
    toBack (figure: Figure) {
      ...
      if (this.composite instanceof StrongComposite) {
        this.toFront(<Figure><unknown>this.composite)
        return
      }
    ...
    installEditPolicy (policy: EditPolicy) {
      ...
      //
      this.editPolicy.grep((p: EditPolicy) => {
        let stay = p.constructor.name !== policy.constructor.name;
     ...
     * @since 4.81
     */
    uninstallEditPolicy (policy: EditPolicy) {
      ...
      //
      let name = (isStringObject(policy)) ? policy : policy.constructor.name
      this.editPolicy.grep((p) => {
        if (p.NAME === name) {
          p.onUninstall(this)
          return false
    ...
    repaint (attributes?: any) {
      ...
      //
      attributes = JSON.flatDiff(attributes, this.lastAppliedAttributes)
     ...
     */
    getHandleBBox (): Rectangle | ArrayList {
      return null
    }
    /**
     *
    ...
    onDragStart (x: number, y: number, shiftKey: boolean, ctrlKey: boolean) {
      ...
      //
      // @since 5.6.0
      let bbox = <Rectangle>this.getHandleBBox()
      if (bbox !== null && bbox.translate(this.getAbsolutePosition().scale(-1)).hitTest(x, y) === false) {
        ...
        this.panningDelegate = this.getBestChild(<number>this.x + x, this.getY() + y)
      ...
      this.command = this.createCommand(CommandType.MOVE)
     ...
     * the super implementation via <code>super(dx, dy, dx2, dy2);</code>
     *
     * @private
     ...
     * @param {Boolean} ctrlKey true if the ctrl key has been pressed during the event
     **/
    onDrag (dx: number, dy: number, dx2: number, dy2: number, shiftKey: boolean, ctrlKey: boolean) {
     ...
     * Sub classes can override this method to implement additional stuff. Don't forget to call
     * the super implementation via <code>super();</code>
     *
     ...
     * @since 5.0.0
     */
    on (event: string, callback?: (e: any) => void, context?: any) {
     ...
     **/
    getBestChild (x: number, y: number, figureToIgnore?: Figure | any[]) : Figure | any[] {
